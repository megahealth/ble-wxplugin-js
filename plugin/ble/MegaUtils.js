const  md5 =require("./MegaMD5") ;
const  aes =require('./MegaAes') 
...
const byte4ToInt = (a) => (a[3] << 24) | (a[2] << 16) | (a[1] << 8) | a[0]
...
const encryptMac = mac => _encrypt(macToBytes(mac))
const encryptToken = token => _encrypt(tokenToBytes(token))

const getDfuMac = mac => mac.slice(0, -2) + ('0x00' + (+("0x" + mac.slice(-2)) + 1).toString(16)).slice(-2).toUpperCase().slice(-2);


const discoverServicesAndChs = deviceId => {
...
function bytesToUint8Array (b,swVer) {
  if (b[0] === 94 || b[0] === 91) {
    let array=[]
